...
@section('content')
    ...
    <div class="container">
        <div class="row col-md-6">
            <form method="POST" action="/personels/{{ $competenceTechnique->id }}/edit">
                @csrf
                @method('PATCH')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="offre_adresse"
                        value="{{ $competenceTechnique->nom_de_la_competence }}" id="exampleInputEmail1"
            ...
            <form action="/techniques/{{ $competenceTechnique->id }}" method="post" class="hidden" id="delete-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
